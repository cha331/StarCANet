...
class Trainer:
    ...
    def __init__(self, model, device, optimizer, criterion, scheduler, folder_name,
                 resume=None, print_freq=50, val_freq=10, test_freq=5):
        ...
        self.save_path = f'./exp/{self.folder_name}'
        ...
        self.train_writer = SummaryWriter(log_dir=self.save_path)
        self.test_writer = SummaryWriter(log_dir=self.save_path)
    ...
    def loop(self, epochs, train_loader, val_loader, test_loader):
        ...
        for ep in range(self.cur_epoch, epochs + 1):
            ...
            self.train_writer.add_scalar(f"train lr", self.scheduler.get_lr()[0], global_step=ep)
            self.train_writer.add_scalar(f"loss/train", self.train_loss, global_step=ep)

            if ep % self.val_freq == 0:
                self.val_loss = self.val(val_loader)
                self.train_writer.add_scalar(f"loss/val", self.val_loss, global_step=ep)

            if ep % self.test_freq == 0:
                self.test_loss, rho, nmse = self.test(test_loader)
                self.test_writer.add_scalar("loss/test",self.test_loss,global_step=ep)
                self.test_writer.add_scalar("test rho", rho, global_step=ep)
                self.test_writer.add_scalar("test nmse",nmse,global_step=ep)
            else:
                rho, nmse = None, None
    ...
    def test(self, test_loader):
        ...
        self.model.eval()
        with torch.no_grad():
            return self.tester(test_loader)
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx, (sparse_gt, ) in enumerate(data_loader):
            ...
            # if self.model.training:
            #     # record train lr & loss every iter
            #     global_step = (self.cur_epoch - 1) * len(data_loader) + (batch_idx + 1)
            #     train_writer.add_scalar(f"train lr", self.scheduler.get_lr()[0], global_step=global_step)
            #     train_writer.add_scalar(f"train loss", iter_loss.avg, global_step=global_step)
    ...
    def _loop_postprocessing(self, rho, nmse):
        ...
        if rho is not None:
            if self.best_rho.rho is None or self.best_rho.rho < rho:
                ...
                self._save(state, name=f"best_rho_{self.folder_name}.pth")
            if self.best_nmse.nmse is None or self.best_nmse.nmse > nmse:
                ...
                self._save(state, name=f"best_nmse_{self.folder_name}.pth")

        self._save(state, name=f'last_{self.folder_name}.pth')
...
class Tester:
    ...
    def _iteration(self, data_loader):
        ...
        logger.info(f'\n=> Test Result:\n'
                    f'loss: {iter_loss.avg:.3e}  rho:{iter_rho.avg:.3e}  NMSE: {iter_nmse.avg:.3e}\n')
